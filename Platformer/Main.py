...
def gameLoop():
    ...
    class Platform(object):
        def __init__(self, x, y, width, height, texture=dirt_block, collisions=True):
            ...
            self.hardness = 1
            self.break_level = self.hardness * 180
    ...
    platforms = [Platform(0, 0, 0, height, False),
    ...
    for x in range(0, 7000, 40):
        platforms.append(Platform(x, 720, 40, 40, grass_block))
    for x in range(0, 7000, 40):
        platforms.append(Platform(x, 760, 40, 40))
    for x in range(0, 7000, 40):
        for y in range(800, 961, 40):
            platforms.append(Platform(x, y, 40, 40, stone_block))
